...
view(laworder_points)

# data --------------------------------------------------------------------
laworder_points = read_sf("Data/Law&Order/laworder_points.geojson")  
...
# FAI_laworder = read_sf("FAI/Law&Order/FAI_laworder.geojson")  # shortcut, no new calculation

# laworder data of neighborhoods, saved on disc ----------------------------------------------------------
...
result = map(1:250, function(i){  # für jeden zbez
  ...
  laworder_in_zbez = laworder_points[zbez, ] %>%
    mutate(inZbez = TRUE)
  
  # find points in isochrone
  laworder_in_isochrone = laworder_points[iso, ] %>%
    mutate(inZbez = FALSE)
  
  #  in isochrone, aber nicht in zbez
  laworder_in_iso_but_no_zbez = anti_join(laworder_in_isochrone, laworder_in_zbez %>% st_drop_geometry(), join_by(name))
  
  # in zbez + in isochrone (aber nicht in zbez)
  laworder_iso_zbez = bind_rows(laworder_in_zbez, laworder_in_iso_but_no_zbez)
  
  
  # um welchen zbez handelt es sich
  laworder_iso_zbez[["zbez"]] = zbez[["ZBEZ"]]
...
all_results_final = right_join(all_results, data_zbez, join_by(zbez == ZBEZ)) |>
  select( "ZBEZ" = zbez, "Police_Iso" = FALSE_police,  
          "Police_Zbez" = TRUE_police) 
...
FAI_laworder <- merge(all_results_final, data_pop, by="ZBEZ") |> 
  mutate( FAI_Zbez = Police_Zbez,     # no weighting of different amenities
          FAI_Iso = Police_Iso, # no weighting of neighboring area
          FAI_total = (FAI_Zbez + FAI_Iso) / POP_TOTAL)
FAI_laworder$FAI_total[is.infinite(FAI_laworder$FAI_total)] <- 0      # because one zbez population = 0  
          
FAI_laworder = FAI_laworder |>
...
FAI_laworder[is.na(FAI_laworder)] <- 0   # to replace NaNs with 0 for plotting

quintile <- quantile(FAI_laworder$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_laworder$quintile <- cut(FAI_laworder$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/Law&Order/FAI_laworder.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_laworder, output_path)
...
# new FAI calculation
FAI_laworder_filtered <- merge(all_results_final, data_pop_filtered, by="ZBEZ") |> 
  mutate( FAI_Zbez = Police_Zbez ,     # no weighting of different amenities
          FAI_Iso = Police_Iso , # no weighting of neighboring area
...
quintile <- quantile(FAI_laworder_filtered$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_laworder_filtered$quintile <- cut(FAI_laworder_filtered$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/Law&Order/FAI_laworder_filtered.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_laworder_filtered, output_path)
...
ggplot() +
  ...
  geom_sf(data = FAI_laworder_filtered, aes(fill= quintile),
          color = "grey20") +
    geom_sf(data = laworder_points, 
            aes(shape = type), 
            size = 0.6, 
            show.legend = "point") +
  theme_map() +
...
# save image
ggsave(file = "FAI/Overview/FAI_laworder_filtered.png", plot = last_plot(), width = 10, height = 8, units = "in")
...
ggplot() +
  geom_sf(data = FAI_laworder, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = laworder_points, 
          aes(shape = type), 
          size = 0.4) +
  ...
  scale_fill_continuous(type = "viridis",
                        direction = -1) +
  ggtitle("Access to laworder in Vienna") +
  scale_shape_discrete() +  
...
laworder_points_agg <- data_bez |> 
  mutate(count = lengths(st_intersects(data_bez, laworder_points)))
...
FAI_laworder_agg <- merge(laworder_points_agg, data_pop_agg, by = "BEZ") |> 
...
quintile <- quantile(FAI_laworder_agg$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
FAI_laworder_agg$quintile <- cut(FAI_laworder_agg$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/laworder/FAI_laworder_agg.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_laworder_agg, output_path)

# plotting
ggplot()  +
  geom_sf(data = FAI_laworder_agg, aes(fill= quintile),
          color = "grey20", 
          size = 0.4) +
  geom_sf(data = laworder_points, 
          aes(shape = type), 
...
# save image
ggsave(file = "FAI/Overview/FAI_laworder_agg.png", plot = last_plot(), width = 10, height = 8, units = "in")

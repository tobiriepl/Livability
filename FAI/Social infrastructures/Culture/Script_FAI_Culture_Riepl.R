...
# data --------------------------------------------------------------------
culture_points = read_sf("Data/Culture/culture_points.geojson")  
...
# FAI_culture = read_sf("FAI/culture/FAI_culture.geojson")  # shortcut, no new calculation



# culture data of neighborhoods, saved on disc ----------------------------------------------------------
result = map(1:250, function(i){  # für jeden zbez
  ...
  # find points in zbez, similar to: st_intersection(data_points, zbez)
   culture_in_zbez = culture_points[zbez, ] %>%
    mutate(
      inZbez = TRUE
    )
  
  # find points in isochrone
  culture_in_isochrone = culture_points[iso, ] %>%
    mutate(
      inZbez = FALSE
    )
  
  #  in isochrone, aber nicht in zbez
  culture_in_iso_but_no_zbez = anti_join(culture_in_isochrone, culture_in_zbez %>% st_drop_geometry(), join_by(name))
  
  # in zbez + in isochrone (aber nicht in zbez)
  culture_iso_zbez = bind_rows(culture_in_zbez, culture_in_iso_but_no_zbez)
  
  
  # um welchen zbez handelt es sich
  culture_iso_zbez[["zbez"]] = zbez[["ZBEZ"]]
...
# append the ZBEZ number of the missing ones: the ones filtered out above, will have 0s in all columns
all_results_final = right_join(all_results, data_zbez, join_by(zbez == ZBEZ)) |>
  select( "ZBEZ" = zbez, "Museum_Iso" = FALSE_museum, "Theatre_Iso" = FALSE_theatre, "Cinema_Iso" = FALSE_cinema, "Attraction_Iso" = FALSE_attraction, "Community_Centre_Iso" = "FALSE_community centre", "Event_Venue_Iso" = "FALSE_event venue",
          "Museum_Zbez" = TRUE_museum,  "Theatre_Zbez" = TRUE_theatre, "Cinema_Zbez" = TRUE_cinema, "Attraction_Zbez" = TRUE_attraction, "Community_Centre_Zbez" ="TRUE_community centre","Event_Venue_Zbez" = "TRUE_event venue")
...
#FAI calculation
FAI_culture <- merge(all_results_final, data_pop, by="ZBEZ") |> 
  mutate( FAI_Zbez = Museum_Zbez  + Theatre_Zbez + Cinema_Zbez   + Attraction_Zbez + Community_Centre_Zbez + Event_Venue_Zbez,     # no weighting of different amenity types
          FAI_Iso = (Museum_Iso + Theatre_Iso + Cinema_Iso  + Attraction_Iso + Community_Centre_Iso + Event_Venue_Iso) * 1, # no weighting of different neighborhoods
          FAI_total = (FAI_Zbez + FAI_Iso) / POP_TOTAL)
FAI_culture$FAI_total[is.infinite(FAI_culture$FAI_total)] <- 0  

FAI_culture = FAI_culture |>
...
FAI_culture[is.na(FAI_culture)] <- 0   # to replace NaNs with 0 for plotting

quintile <- quantile(FAI_culture$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_culture$quintile <- cut(FAI_culture$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = "FAI/Culture/FAI_culture.geojson"
# delete old version
unlink(output_path)
write_sf(FAI_culture, output_path)


# FIGURE FOR ARTICLE

# filter population and then calculate FAI
...
# new FAI calculation
FAI_culture_filtered <- merge(all_results_final, data_pop_filtered, by="ZBEZ") |> 
  mutate( FAI_Zbez = Museum_Zbez  + Theatre_Zbez + Cinema_Zbez   + Attraction_Zbez + Community_Centre_Zbez + Event_Venue_Zbez,     # no weighting of different amenity types
          FAI_Iso = (Museum_Iso + Theatre_Iso + Cinema_Iso  + Attraction_Iso + Community_Centre_Iso + Event_Venue_Iso) * 1, # no weighting of different neighborhoods
...
quintile <- quantile(FAI_culture_filtered$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_culture_filtered$quintile <- cut(FAI_culture_filtered$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = "FAI/Culture/FAI_culture_filtered.geojson"
# delete old version
unlink(output_path)
write_sf(FAI_culture_filtered, output_path)
...
ggplot() +
  ...
  geom_sf(data = FAI_culture_filtered, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = culture_points, 
          aes(shape = type), 
          size = 0.6, 
          show.legend = "point") +
  theme_map() +
...
# save image
ggsave(file = "FAI/Overview/FAI_culture_filtered.png", plot = last_plot(), width = 10, height = 8, units = "in")
...
ggplot() +
  geom_sf(data = FAI_culture, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = culture_points, 
          aes(shape = type), 
          size = 0.6, 
  ...
  scale_fill_continuous(type = "viridis",
                        direction = -1) +
  ggtitle("Access to culture in Vienna") +
  scale_shape_discrete() +  
  theme(legend.position = "left", plot.title = element_text(hjust = 0.2, vjust = -30)) +
  geom_text(data = FAI_culture, aes(x = st_coordinates(st_centroid(FAI_culture))[, "X"], y = st_coordinates(st_centroid(FAI_culture))[, "Y"], label = ZBEZ), size = 2) 
...
culture_points_agg <- data_bez |> 
  mutate(count = lengths(st_intersects(data_bez, culture_points)))
...
FAI_culture_agg <- merge(culture_points_agg, data_pop_agg, by = "BEZ") |> 
...
quintile <- quantile(FAI_culture_agg$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
FAI_culture_agg$quintile <- cut(FAI_culture_agg$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# plotting
ggplot()  +
  geom_sf(data = FAI_culture_agg, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = culture_points, 
          aes(shape = type), 
          size = 0.6,
...
# save image
ggsave(file = "FAI/Overview/FAI_culture_agg.png", plot = last_plot(), width = 10, height = 8, units = "in")

...
# data --------------------------------------------------------------------
post_points = read_sf("Data/Post/post_points.geojson")  
...
# FAI_post = read_sf("FAI/Post/FAI_post.geojson")  # shortcut, no new calculation
...
result = map(1:250, function(i){  # für jeden zbez
  ...
  post_in_zbez = post_points[zbez, ] %>%
    mutate(inZbez = TRUE)
  
  # find points in isochrone
  post_in_isochrone = post_points[iso, ] %>%
    mutate(inZbez = FALSE)
  
  #  in isochrone, aber nicht in zbez
  post_in_iso_but_no_zbez = anti_join(post_in_isochrone, post_in_zbez %>% st_drop_geometry(), join_by(name))
  
  # in zbez + in isochrone (aber nicht in zbez)
  post_iso_zbez = bind_rows(post_in_zbez, post_in_iso_but_no_zbez)
  
  
  # um welchen zbez handelt es sich
  post_iso_zbez[["zbez"]] = zbez[["ZBEZ"]]
...
all_results_final = right_join(all_results, data_zbez, join_by(zbez == ZBEZ)) |>
  select( "ZBEZ" = zbez, "Post_office_Iso" = "FALSE_post office", 
          "Post_office_Zbez" = "TRUE_post office") 
...
FAI_post <- merge(all_results_final, data_pop, by="ZBEZ") |> 
  mutate( FAI_Zbez = Post_office_Zbez,     # no weighting of different amenities
          FAI_Iso = Post_office_Iso, # no weighting of neighboring area
          FAI_total = (FAI_Zbez + FAI_Iso) / POP_TOTAL)
FAI_post$FAI_total[is.infinite(FAI_post$FAI_total)] <- 0      # because one zbez population = 0  
...
FAI_post[is.na(FAI_post)] <- 0   # to replace NaNs with 0 for plotting
...
# write to disk
output_path = here("FAI/Post/FAI_post.geojson")
# delete old version
unlink(output_path)
...
# new FAI calculation
FAI_post_filtered <- merge(all_results_final, data_pop_filtered, by="ZBEZ") |> 
  mutate( FAI_Zbez = Post_office_Zbez,     # no weighting of different amenities
          FAI_Iso = Post_office_Iso, # no weighting of neighboring area
...
quintile <- quantile(FAI_post_filtered$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_post_filtered$quintile <- cut(FAI_post_filtered$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/Post/FAI_post_filtered.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_post_filtered, output_path)
...
ggplot() +
  ...
  geom_sf(data = FAI_post_filtered, aes(fill= quintile),
          color = "grey20") +
    geom_sf(data = post_points, 
            aes(shape = type), 
            size = 0.6, 
            show.legend = "point") +
  theme_map() +
...
# save image
ggsave(file = "FAI/Overview/FAI_post_filtered.png", plot = last_plot(), width = 10, height = 8, units = "in")
...
ggplot() +
  geom_sf(data = FAI_post, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = post_points, 
          aes(shape = type), 
          size = 0.4) +
  ...
  scale_fill_continuous(type = "viridis",
                        direction = -1) +
  ggtitle("Access to post in Vienna") +
  scale_shape_discrete() +  
...
post_points_agg <- data_bez |> 
  mutate(count = lengths(st_intersects(data_bez, post_points)))
...
FAI_post_agg <- merge(post_points_agg, data_pop_agg, by = "BEZ") |> 
...
quintile <- quantile(FAI_post_agg$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
FAI_post_agg$quintile <- cut(FAI_post_agg$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/post/FAI_post_agg.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_post_agg, output_path)

# plotting
ggplot()  +
  geom_sf(data = FAI_post_agg, aes(fill= quintile),
          color = "grey20", 
          size = 0.4) +
  geom_sf(data = post_points, 
          aes(shape = type), 
...
# save image
ggsave(file = "FAI/Overview/FAI_post_agg.png", plot = last_plot(), width = 10, height = 8, units = "in")

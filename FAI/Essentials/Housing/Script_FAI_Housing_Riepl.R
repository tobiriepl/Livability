...
# data --------------------------------------------------------------------
housing_points = read_sf("Data/Housing/housing_points.geojson")  
...
# FAI_housing = read_sf("FAI/Housing/FAI_housing.geojson")  # shortcut, no new calculation
...
result = map(1:250, function(i){  # für jeden zbez
  ...
  housing_in_zbez = housing_points[zbez, ] %>%
    mutate(inZbez = TRUE)
  
  # find points in isochrone
  housing_in_isochrone = housing_points[iso, ] %>%
    mutate(inZbez = FALSE)
  
  #  in isochrone, aber nicht in zbez
  housing_in_iso_but_no_zbez = anti_join(housing_in_isochrone, housing_in_zbez %>% st_drop_geometry(), join_by(name))
  
  # in zbez + in isochrone (aber nicht in zbez)
  housing_iso_zbez = bind_rows(housing_in_zbez, housing_in_iso_but_no_zbez)
  
  
  # um welchen zbez handelt es sich
  housing_iso_zbez[["zbez"]] = zbez[["ZBEZ"]]
...
all_results_final = right_join(all_results, data_zbez, join_by(zbez == ZBEZ)) |>
  select( "ZBEZ" = zbez, "Municipal_Iso" = FALSE_municipal, 
          "Municipal_Zbez" = TRUE_municipal) 
...
FAI_housing <- merge(all_results_final, data_pop, by="ZBEZ") |> 
  mutate( FAI_Zbez = Municipal_Zbez,
...
# write to disk
output_path = here("FAI/Housing/FAI_housing.geojson")
# delete old version
unlink(output_path)
...
# new FAI calculation
FAI_housing_filtered <- merge(all_results_final, data_pop_filtered, by="ZBEZ") |> 
  mutate( FAI_Zbez = Municipal_Zbez,
          FAI_Iso = Municipal_Iso,
...
quintile <- quantile(FAI_housing_filtered$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
# quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_housing_filtered$quintile <- cut(FAI_housing_filtered$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/housing/FAI_Housing_filtered.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_housing_filtered, output_path)
...
ggplot() +
  ...
  geom_sf(data = FAI_housing_filtered, aes(fill= quintile),
          color = "grey20") +
    geom_sf(data = housing_points, 
            aes(shape = type), 
            size = 0.6, 
            show.legend = "point") +
  theme_map() +
...
# save image
ggsave(file = "FAI/Overview/FAI_Housing_filtered.png", plot = last_plot(), width = 10, height = 8, units = "in")
...
ggplot() +
  geom_sf(data = FAI_housing, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = housing_points, 
          aes(shape = type), 
          size = 0.4) +
  ...
  scale_fill_continuous(type = "viridis",
                        direction = -1) +
  ggtitle("Access to housing in Vienna") +
  scale_shape_discrete() +  
...
housing_points_agg <- data_bez |> 
  mutate(count = lengths(st_intersects(data_bez, housing_points)))
...
FAI_housing_agg <- merge(housing_points_agg, data_pop_agg, by = "BEZ") |> 
...
quintile <- quantile(FAI_housing_agg$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
FAI_housing_agg$quintile <- cut(FAI_housing_agg$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/housing/FAI_housing_agg.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_housing_agg, output_path)

# plotting
ggplot()  +
  geom_sf(data = FAI_housing_agg, aes(fill= quintile),
          color = "grey20", 
          size = 0.4) +
  geom_sf(data = housing_points, 
          aes(shape = type), 
...
# save image
ggsave(file = "FAI/Overview/FAI_housing_agg.png", plot = last_plot(), width = 10, height = 8, units = "in")

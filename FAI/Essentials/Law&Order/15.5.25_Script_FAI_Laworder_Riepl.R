...
# data --------------------------------------------------------------------
laworder_points = read_sf("FAI/Data/Law&Order/laworder_points.geojson")  
...
data_laworder = merge(data_zbez, laworder_points |> st_drop_geometry(), by = "ZBEZ") |>
...
result = map(1:23, function(i){  # für jeden zbez
  ...
  laworder_in_bez = data_laworder[bez, ] %>%
    mutate(inBez = TRUE)
  
  # find points in isochrone
  laworder_in_isochrone = data_laworder[iso, ] %>%
    mutate(inBez = FALSE)
  
  #  in isochrone, aber nicht in zbez
  laworder_in_iso_but_no_bez = anti_join(laworder_in_isochrone, laworder_in_bez %>% st_drop_geometry(), join_by(name))
  
  # in bez + in isochrone (aber nicht in bez)
  laworder_iso_bez = bind_rows(laworder_in_bez, laworder_in_iso_but_no_bez)
  
  
  # um welchen zbez handelt es sich
  laworder_iso_bez[["bez"]] = bez[["BEZ"]]
...
all_results_final = right_join(all_results, data_bez, join_by(bez == BEZ)) |>
  select( "BEZ" = bez, "Police_Iso" = FALSE_police,  
          "Police_Bez" = TRUE_police) 
...
FAI_laworder <- merge(all_results_final, data_pop, by="BEZ") |> 
  distinct(BEZ, .keep_all = TRUE) |>
  
  mutate( FAI_Bez = Police_Bez,     # no weighting of different amenities
...
# write to disk
output_path = here("FAI/Essentials/Law&Order/FAI_laworder.geojson")
# delete old version
unlink(output_path)

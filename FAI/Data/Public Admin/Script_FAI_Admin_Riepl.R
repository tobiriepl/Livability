...
# data --------------------------------------------------------------------
admin_points = read_sf("Data/admin/admin_points.geojson")  
...
# FAI_admin = read_sf("FAI/admin/FAI_admin.geojson")  # shortcut, no new calculation
...
result = map(1:250, function(i){  # für jeden zbez
  ...
  admin_in_zbez = admin_points[zbez, ] %>%
    mutate(inZbez = TRUE)
  
  # find points in isochrone
  admin_in_isochrone = admin_points[iso, ] %>%
    mutate(inZbez = FALSE)
  
  #  in isochrone, aber nicht in zbez
  admin_in_iso_but_no_zbez = anti_join(admin_in_isochrone, admin_in_zbez %>% st_drop_geometry(), join_by(name))
  
  # in zbez + in isochrone (aber nicht in zbez)
  admin_iso_zbez = bind_rows(admin_in_zbez, admin_in_iso_but_no_zbez)
  
  
  # um welchen zbez handelt es sich
  admin_iso_zbez[["zbez"]] = zbez[["ZBEZ"]]
...
all_results_final = right_join(all_results, data_zbez, join_by(zbez == ZBEZ)) |>
  select( "ZBEZ" = zbez, "Admin_office_Iso" = "FALSE_admin office", "Admin_office_Zbez" = "TRUE_admin office")
...
FAI_admin <- merge(all_results_final, data_pop, by="ZBEZ") |> 
  mutate( FAI_Zbez = Admin_office_Zbez, 
...
# write to disk
output_path = here("FAI/admin/FAI_admin.geojson")
# delete old version
unlink(output_path)
...
# new FAI calculation
FAI_admin_filtered <- merge(all_results_final, data_pop_filtered, by="ZBEZ") |> 
  mutate( FAI_Zbez = Admin_office_Zbez,      # no weighting of different amenities
          FAI_Iso = Admin_office_Iso, # no weighting of neighboring area
...
quintile <- quantile(FAI_admin_filtered$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
quintile <- quintile + seq(0, 0.001, length.out = length(quintile))
FAI_admin_filtered$quintile <- cut(FAI_admin_filtered$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/admin/FAI_admin_filtered.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_admin_filtered, output_path)

View(FAI_admin_filtered)
...
ggplot() +
  ...
  geom_sf(data = FAI_admin_filtered, aes(fill = quintile),
          color = "grey20") +
  #  geom_sf(data = admin_points, 
  #          aes(shape = type), 
  #          size = 0.6, 
  #          show.legend = "point") +
  theme_map() +
...
# save image
ggsave(file = "FAI/Overview/FAI_admin_filtered.png", plot = last_plot(), width = 10, height = 8, units = "in")
...
ggplot() +
  geom_sf(data = FAI_admin, aes(fill= quintile),
          color = "grey20") +
  geom_sf(data = admin_points, 
          aes(shape = type), 
          size = 0.4) +
  ...
  scale_fill_continuous(type = "viridis",
                        direction = -1) +
  ggtitle("Access to admin in Vienna") +
  scale_shape_discrete() +  
...
admin_points_agg <- data_bez |> 
  mutate(count = lengths(st_intersects(data_bez, admin_points)))
...
FAI_admin_agg <- merge(admin_points_agg, data_pop_agg, by = "BEZ") |> 
...
quintile <- quantile(FAI_admin_agg$FAI_final, probs = seq(0, 1, by = 0.2), na.rm = TRUE)
FAI_admin_agg$quintile <- cut(FAI_admin_agg$FAI_final, breaks = quintile, labels = FALSE, include.lowest = TRUE)

# write to disk
output_path = here("FAI/admin/FAI_admin_agg.geojson")
# delete old version
unlink(output_path)
write_sf(FAI_admin_agg, output_path)

# plotting
ggplot()  +
  geom_sf(data = FAI_admin_agg, aes(fill= quintile),
          color = "grey20", 
          size = 0.4) +
  geom_sf(data = admin_points, 
          aes(shape = type), 
...
# save image
ggsave(file = "FAI/Overview/FAI_admin_agg.png", plot = last_plot(), width = 10, height = 8, units = "in")
